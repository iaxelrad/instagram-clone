...
describe('Dashboard', () => {
  ...
  it('logs the user in and shows the dashboard and does basic checks around the UI', () => {
    cy.get('body').within(() => {
      cy.get('div').should('contain.text', 'itamar'); // username in the sidebar
      cy.get('div').should('contain.text', 'Itamar Axelrad'); // full name in the sidebar
  ...
  it('logs the user in and add a comment to a photo', () => {
    cy.get('[data-testid="add-comment-AX76hnRttW2sn1j99W9v"]')
      .should('have.attr', 'placeholder', 'Add a comment...')
      .type('Amazing photo!');
    cy.get('[data-testid="add-comment-submit-AX76hnRttW2sn1j99W9v"]').submit();
  });

  it('logs the user in and likes a photo', () => {
    cy.get('[data-testid="like-photo-AX76hnRttW2sn1j99W9v"]').click();
  });
